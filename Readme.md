...
We have developed a web app for providing a user friendly platform to the users. 
...
While using the web-app, **[experiment app]** is used instead of **[experiment name]** and **[data paths app]** instead of **[data paths]**. 

## For running the non-app version
Run testing by:
```
...
python server.py
```
Go to the link of the local server. Then select the corresponding retinal image, border mask and the ground truth image(for comparison purpose) and upload. 
